        ...
        public int Move(bool trivia)
        {
            if (shouldTeleport())
            {
               /* // teleport to random room, not including one that was just in
                int currentWumpusRoom = m.getWumpusRoom();
                do {
                    m.randomizeWumpusRoom(new Room[1]);
                } while (currentWumpusRoom == m.getWumpusRoom());

                numberOfMoves = -1;
                WumpusState = state.asleep;*/
            }
                // What happens if Wumpus loses due to trivia
            else if (trivia)
            {
                // 2-4 rooms?
