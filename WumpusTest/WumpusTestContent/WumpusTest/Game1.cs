    ...
    public class Game1 : Microsoft.Xna.Framework.Game
    {
        ...
        */
        ...
        private void UpdateAngle()
        {
            ...
            //Vector2 Direction = new Vector2(
            //    MyGameControl.MyPlayer.Position.X + CenteredSprite.height / 2 - (MState.X + Sprite.stdHeight / 2),
            //    MyGameControl.MyPlayer.Position.Y + CenteredSprite.height / 2 - (MState.Y + Sprite.stdHeight / 2));
        ...
        private void UpdateKinectClicks()
        {
            ...
            else if (MyGameControl.GameState == GameControl.Gstate.Menu && MyMenu.MyMenuState == Menu.MenuState.Options1)
            {
        ...
        private void UpdateMouseClicks(MouseState OldState)
        {
            ...
            if (NewState.LeftButton == ButtonState.Pressed && OldState.LeftButton == ButtonState.Released)
            {
                ...
                else if (MyMenu.MyMenuState == Menu.MenuState.Options1)
                {
        ...
        {
            MyMenu.MyMenuState = Menu.MenuState.Options1;
        }
        ...
        private void Cave2Click()
        {
            if (MyGameControl.caveN != 2)
            {
                ...
                MyGameControl.MyCave = new Cave2("C:\\Users\\" + MyGameControl.username + "\\Dropbox\\WumpusTest\\WumpusTest\\WumpusTestContent\\CaveFiles\\MapDataCave1.txt");
        ...
        public Direction getDirectionFromXY(Vector2 Position)
        {
            ...
            if (Position.X < Room.thisSize.Left + thresholdDistance)
                return Direction.West;
            else if (Position.X > Room.thisSize.Right - thresholdDistance)
                return Direction.East;
            else if (Position.Y < Room.thisSize.Top + thresholdDistance)
                return Direction.North;
            else if (Position.Y > Room.thisSize.Bottom - thresholdDistance)
        ...
        private void DrawText()
        {
            if (MyGameControl.isDebugMode)
            {
                ...
                Debug += "\nRoom Number: " + CurrentRoom.getIntegerForm();
                Debug += "\n" + CurrentRoom;
                Debug += "\nWumpus: " + MyGameControl.MyMap.getWumpusRoom();
                Debug += "\n" + new Room(MyGameControl.MyMap.getWumpusRoom());
                spriteBatch.DrawString(standardFont, Debug, new Vector2(10, 470), Color.White);
            }

            if (MyGameControl.GameState == GameControl.Gstate.Game)
            {
                spriteBatch.DrawString(standardFont, MyGameControl.MyMap.getPlayerRoom().ToString(), new Vector2(Room.RoomSize.Center.X, Room.RoomSize.Center.Y), Color.White);

                decimal time = Math.Round((decimal)(MyWatch.Elapsed.TotalSeconds), 3);
                // ADD SCORE
                spriteBatch.DrawString(standardFont, "Time: " + (time), new Vector2(460, 550), Color.White);
                spriteBatch.DrawString(standardFont, "Moves: " + MyGameControl.MyMap.moves, new Vector2(620, 550), Color.White);
